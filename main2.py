...
from calcchannel_atg import calculate_channel
import random
...
t = 40
for i in range(s):
    taxa = 35#random.randint(35, 40) # t = 400 * 1024;  # Taxa Requerida 400 Kbps
...
posx = [540, 660, 780]
posy = [530, 650, 730]
h=300 #valor multiplicado por 10
for i in range(uav):
...
for i in alluser:
    print("Status: ", i.C, "ID:", i.ID, i.X, i.Y, i.DR, i.R_DR, "UAV: ", i.EB, i.PRB, i.CQI, i.SINR, i.PRX, i.V, i.M,
          i.ES, i.EBC, i.Int,"pathloss",i.PRX, "SINR",i.SINR, "Data Rate:",i.DR/ 10**6,"Mbps")
    if i.SINR:
        nub=nub+1
        mediaSIN=mediaSIN+i.SINR
        mediapath = mediapath + i.PRX
        mediadr = mediadr + i.DR
...
print('MediaTroughput =', mediadr/nub)
